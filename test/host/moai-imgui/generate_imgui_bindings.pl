...
while ($line = <STDIN>) {
  ...
  if ($line =~ m/ *IMGUI_API *([^ ]+) *([^\(]+)\(([^\;]*)\);/) {
    ...
    for (my $i = 0; $i < @args; $i++) {
      ...
      } elsif ($args[$i] =~ m/^ *(ImGuiWindowFlags|ImGuiCol|ImGuiStyleVar|ImGuiKey|ImGuiAlign|ImGuiColorEditMode|ImGuiMouseCursor|ImGuiSetCond|ImGuiTreeNodeFlags|ImGuiInputTextFlags|ImGuiSelectableFlags) ([^ ]*)( = 0|) *$/) {
       #These are ints
       my $name = $2;
        if ($3 =~ m/^ = 0$/) {
