...
void MOAISpineSkeletonSimple::Draw ( int subPrimID, float lod ) {
	...
	for (int i = 0, n = this->mSkeleton->slotsCount; i < n; i++) {
...
static const unsigned short _quadTriangles[6] = {0, 1, 2, 2, 3, 0};
//----------------------------------------------------------------//
void MOAISpineSkeletonSimple::DrawSpineSlot ( Slot* slot ) {
	...
	} else if( slot->attachment->type == ATTACHMENT_WEIGHTED_MESH ) {
		WeightedMeshAttachment* attachment = (WeightedMeshAttachment*)slot->attachment;
		WeightedMeshAttachment_computeWorldVertices(
			attachment, slot, worldVertices
		);
		texture        = (MOAITexture*) ((AtlasRegion*) attachment->rendererObject) ->rendererObject;
		uvs            = attachment->uvs;
	...
	switch( slot->data->blendMode ){
		...
		case SP_BLEND_MODE_MULTIPLY:
			gfxDevice.SetBlendMode ( ZGL_BLEND_FACTOR_DST_COLOR, ZGL_BLEND_FACTOR_ZERO );
			break;
		case SP_BLEND_MODE_SCREEN:
			gfxDevice.SetBlendMode ( ZGL_BLEND_FACTOR_ONE, ZGL_BLEND_FACTOR_ONE_MINUS_SRC_COLOR );
			break;
		}
...
void MOAISpineBoneHandle::SyncTransform() {	
	...
	if( (this->mLockFlags & MOAISpineBoneHandle::LOCK_ROT) == 0 ) {
		this->SetRot( 0.0f, 0.0f, Bone_getWorldRotationX( mBone ) );
	}	else {
	...
	if( (this->mLockFlags & MOAISpineBoneHandle::LOCK_SCL) == 0 ) {
		this->SetScl( Bone_getWorldScaleX( mBone), Bone_getWorldScaleY( mBone ), 1.0f );
	}	else  {
...
int MOAISpineSlot::_getBoneWorldScl ( lua_State *L ) {
	MOAI_LUA_SETUP( MOAISpineSlot, "U" )
	Bone* bone = self->mSlot->bone;
	state.Push( Bone_getWorldScaleX( bone ) );
	state.Push( Bone_getWorldScaleY( bone ) );
	return 2;
}
...
int MOAISpineSlot::_getBoneWorldRot ( lua_State *L ) {
	MOAI_LUA_SETUP( MOAISpineSlot, "U" )
	Bone* bone = self->mSlot->bone;
	state.Push( Bone_getWorldRotationX( bone ) );
	return 1;
}
...
u32 MOAISpineSlot::GetAttachmentBounds  ( ZLBox& bounds ) {
	...
	// } else if( mSlot->attachment->type == ATTACHMENT_WEIGHTED_MESH ) {
	// }
	// return BOUNDS_OK;
	return BOUNDS_GLOBAL;
}
...
void MOAISpineSlot::SetSlot( Slot* slot ) {
	...
	if( mSlot->attachment->type == ATTACHMENT_REGION ) {
		...
		WeightedMeshAttachment *mesh     = (WeightedMeshAttachment*)slot->attachment;
		mTexture.Set( *this,
			(MOAITexture*) ((AtlasRegion*) mesh->rendererObject) ->rendererObject
		);
	}
...
void MOAISpineSlot::DrawRegionAttachment () {
	...
	RegionAttachment_computeWorldVertices( region, slot->bone, worldVertices );
	this->LoadGfxState ();
	this->LoadVertexTransform ();
	// gfxDevice.SetGfxState( texture );
	...
	gfxDevice.WriteVtx ( worldVertices[VERTEX_X1], worldVertices[VERTEX_Y1], 0.0f );
		gfxDevice.WriteUV ( region->uvs[VERTEX_X1], region->uvs[VERTEX_Y1] );
		gfxDevice.WriteFinalColor4b ();

	gfxDevice.WriteVtx ( worldVertices[VERTEX_X2], worldVertices[VERTEX_Y2], 0.0f );
		gfxDevice.WriteUV ( region->uvs[VERTEX_X2], region->uvs[VERTEX_Y2] );
		gfxDevice.WriteFinalColor4b ();
		
	gfxDevice.WriteVtx ( worldVertices[VERTEX_X4], worldVertices[VERTEX_Y4], 0.0f );
	...
	gfxDevice.WriteVtx ( worldVertices[VERTEX_X3], worldVertices[VERTEX_Y3], 0.0f );
...
int MOAISpineSkeleton::_getSlotProps ( lua_State *L ) {
	...
	u32 count = self->mSkeleton->slotsCount;
...
void MOAISpineSkeleton::OnLoad( MOAISpineSkeletonData* data, float ZScale ) {	
	...
	for (int i = 0, n = mSkeleton->slotsCount; i < n; i++) {
		...
		switch( slot->data->blendMode ){
			case SP_BLEND_MODE_NORMAL:
				blendMode.SetBlend ( ZGL_BLEND_FACTOR_SRC_ALPHA, ZGL_BLEND_FACTOR_ONE_MINUS_SRC_ALPHA );
				break;
			case SP_BLEND_MODE_ADDITIVE:
				blendMode.SetBlend ( ZGL_BLEND_FACTOR_SRC_ALPHA, ZGL_BLEND_FACTOR_ONE );
				break;
			case SP_BLEND_MODE_MULTIPLY:
				blendMode.SetBlend ( ZGL_BLEND_FACTOR_DST_COLOR, ZGL_BLEND_FACTOR_ZERO );
				break;
			case SP_BLEND_MODE_SCREEN:
				blendMode.SetBlend ( ZGL_BLEND_FACTOR_ONE, ZGL_BLEND_FACTOR_ONE_MINUS_SRC_COLOR );
				break;
		}
		prop->SetBlendMode( blendMode );
		prop->SetPriority( i );
...
void MOAISpineSkeleton::UpdateSpine() {
	...
	for ( ; slotIt != mSlotPropMap.end (); ++slotIt ) {
		...
		if( (prop->mLockFlags & MOAISpineSlot::LOCK_ROT) == 0 ) {
			...
			rot.mZ = Bone_getWorldRotationX( bone );
			prop->SetRot( rot );
		}
		
		if( (prop->mLockFlags & MOAISpineSlot::LOCK_SCL) == 0 ) {
			ZLVec3D scl = prop->GetScl();
			scl.mX = Bone_getWorldScaleX( bone );
			scl.mY = Bone_getWorldScaleY( bone );
			prop->SetScl( scl );
		}
