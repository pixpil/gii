...
int GIIHelper::_setBufferSize( lua_State *L){
	MOAILuaState state (L);
	u32 w, h;
	MOAIFrameBuffer* fb = state.GetLuaObject < MOAIFrameBuffer >( 1, true );
	
	if ( fb ) {
		w = state.GetValue < u32 >( 2, 1 );
		h = state.GetValue < u32 >( 3, 1 );
		fb->SetBufferSize ( w, h );
	} else {
		w = state.GetValue < u32 >( 1, 1 );
		h = state.GetValue < u32 >( 2, 1 );
